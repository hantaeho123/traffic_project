...
방향은 **노선 + 방면** 단위로 관리합니다. 방면은 CCTV 화면에 찍힌 표지판의 목적지입니다(예: "⬇ 부산 · 서울 ⬆" → 부산 방면, 서울 방면). 한 CCTV 에 노선이 여러 개면(본선 + 램프, 교차 도로) 색 영역마다 노선을 다르게 적습니다.

1. **마스크(등록 2단계)**: 보이는 방면마다 다른 색으로 차로를 칠하고, 색 칩에 방면(예: 서울)을 적습니다.
2. **방면 설정(등록 3단계 / 상세 > 방향 · 지도 표시 / 전국 현황 > 방면 입력하기)**: 방면을 적으면 진행 방향이 자동으로 정해집니다.
   ITS 의 같은 노선 CCTV 좌표를 이어 노선 선을 만들고(`backend/app/services/routes.py`), 목적지(`services/geocode.py`: 내장 표 → OpenStreetMap Nominatim, 캐시)가 선의 어느 쪽에 있는지로 방향을 정합니다.
   목적지가 선 끝 너머에 있거나 한쪽만 풀리면 반대 방향으로 채웁니다. 틀리면 "뒤집기" / "지도에서 직접" 으로 수동 지정합니다.
3. **미측정**: 칠하지 않은 방면은 점유율을 계산하지 않고(`ml/occupancy.py`), DB 에도 쓰지 않으며, 화면에는 "미측정" 으로 표시합니다. 지점 전체 값은 실제로 칠한 방면만으로 계산합니다.

**전국 현황 지도**: 지점 배지(점유율) 아래에 방향별 **회전 화살표 + 숫자**를 붙이고(화살표가 가리키는 쪽으로 진행), 줌 11 이상에서는 도로 위에 방향별 화살표를 그립니다(우측통행이라 진행 방향 오른쪽). 칠하지 않은 방향은 그리지 않습니다. 방면 이름은 방향을 자동으로 정하는 데만 쓰고 지도에는 표시하지 않습니다.

| API | 설명 |
|---|---|
| `POST /api/routes/resolve` | 노선·좌표·방면들 → 진행 방향 |
| `GET /api/routes/near` | 카메라 주변 노선 선 |
| `GET /api/routes/view` | 전국 현황용 노선 선 + 방면별 측정 구간 |
| `PUT /api/cameras/{id}/directions` | 방면·노선·방향 저장 (칠한 마스크는 그대로, 방면이 있으면 방향 자동 계산) |

## 도로 마스크 = 노면 전체 (차량 자리 포함)

점유율의 분모는 **차량이 덮고 있는 자리까지 포함한 노면 전체**여야 합니다. SAM3 "road" 는 보이는 노면만 잡아서 스냅샷에 서 있던 차량 자리가 구멍으로 빠지고,
막힌 도로일수록 분모가 작아져 점유율이 부풀려집니다(광교방음터널 정체 스냅샷: 도로 32.1% → 차량 자리 포함 47.9%).

- **도로 세그멘테이션은 SAM3 만 사용**: "도로 자동 제안" 은 SAM3 를 한 번 호출해 `road` 와 `car, truck, bus` 를 클래스별로 따로 받고, 도로에 닿은 차량만 도로에 합친 뒤 도로로 둘러싸인 작은 구멍을 메웁니다(`backend/app/ml/road_fill.py`). 점·박스 결과에도 SAM3 로 찾은 차량을 같은 규칙으로 합칩니다. YOLO 는 실시간 차량 점유율 계산에만 씁니다.
- 프롬프트를 "road, car, truck, bus" 로 한 덩어리로 합치면 주차장·측도 차량까지 도로(분모)에 들어가므로, 도로에 닿은 차량만 골라 합칩니다.
- 도로와 닿지 않은 차량(주차장 등)과 프레임의 2% 를 넘는 큰 섬(중앙분리대 등)은 채우지 않습니다. SAM 이 한쪽 차로 자체를 못 잡았다면 그 차로는 점/브러시로 따로 칠해야 합니다.
...
- **추론 주기**: 기본은 **5초마다 1장**(`.env` 의 `DEFAULT_INFER_INTERVAL_S`)입니다. 카메라마다 "실시간(연속, `INFER_FPS`)" 또는 "N초마다 1장"(1초~10분)으로 바꿀 수 있습니다(등록 3단계, 표 보기 편집, 상세 페이지). DB 에는 실시간이 `0`, 주기가 초 단위로 저장되고, 값이 비어 있으면 서버 시작 시 기본 주기로 채웁니다.
