...
- **도로 자동 제안 한 번으로 노면 전체**: SAM3 "road" 를 구한 뒤 같은 스냅샷에서 파인튜닝 YOLO(conf 0.15)로 차량을 찾아, 도로에 닿은 차량은 도로에 합치고 도로로 둘러싸인 작은 구멍을 메웁니다(`backend/app/ml/road_fill.py`). 점·박스 결과도 같습니다.
- 프롬프트만 바꾸는 방식은 쓰지 않습니다: "road", "highway", "road with cars" 는 차량 자리의 99% 이상을 빼고, "road, car, truck, bus" 는 도로 밖 물체까지 2.8~3.2%p 더 잡으면서도 차량의 4~11% 를 놓쳤습니다.
