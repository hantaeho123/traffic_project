...
**전국 현황 지도**: 등록 카메라가 있는 노선을 밝은 회색 띠로 그리고, 그 위에 방면마다 한 줄씩 **진행 방향 오른쪽(우측통행)** 에 그 방면의 점유율 색을 칠합니다.
한 CCTV 의 값은 같은 선·같은 방면의 다음 측정 CCTV 와의 중간까지(최대 8km) 칠합니다. 배지 칩은 "서울 12%" 처럼 방면 이름으로, 순위는 "경부선 서울 방면" 처럼 노선·방면 단위로 나옵니다.
노선 선은 CCTV 좌표를 이은 것이라 크게 확대하면 실제 도로와 50~100m 어긋날 수 있습니다.
